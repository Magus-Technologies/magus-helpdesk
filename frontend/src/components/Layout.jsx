...
export default function Layout() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      {/* SIDEBAR */}
      <aside style={{
        width: 228, minWidth: 228, background: 'var(--surface)',
        ...
        <nav style={{ flex: 1, padding: '10px 8px', overflowY: 'auto' }}>
          <style>{`
            ...
            .nav-section {
              ...
              color: var(--muted); padding: 10px 12px 5px; opacity: .7;
            }
          `}</style>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <div style={{
          ...
          />
          <div style={{ flex: 1 }} />
          <button className="btn btn-primary btn-sm" onClick={() => navigate('/tickets/nuevo')}>
            + Nuevo Ticket
          </button>
          ...
          <div style={{ position: 'relative' }}>
            <button onClick={() => setNotifOpen(o => !o)}
              ...
              {sinLeer > 0 && (
                <span style={{
                  position: 'absolute', top: -3, right: -3, background: 'var(--red)',
                  color: '#fff', fontSize: 9, fontWeight: 700, borderRadius: '50%',
                  width: 14, height: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1
            ...
            {notifOpen && (
              <div onClick={e => e.stopPropagation()} style={{
                ...
                borderRadius: 12, width: 320, boxShadow: '0 8px 32px rgba(0,0,0,.3)',
                zIndex: 999, overflow: 'hidden'
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', borderBottom: '1px solid var(--border)' }}>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>Notificaciones</div>
                  {sinLeer > 0 && <button className="btn btn-ghost btn-sm" onClick={marcarTodas} style={{ fontSize: 11 }}>Marcar todas leídas</button>}
                </div>
                <div style={{ maxHeight: 300, overflowY: 'auto' }}>
                  {notifs.length === 0
                    ...
                    : notifs.slice(0, 15).map(n => (
                      <div key={n.id} style={{
                        padding: '10px 14px', borderBottom: '1px solid rgba(42,49,71,.4)',
                        background: n.leida ? 'none' : 'rgba(79,127,255,.05)', cursor: 'pointer'
                        ...
                        <div style={{ fontSize: 10, color: 'var(--muted)', marginTop: 3 }}>{new Date(n.creado_en).toLocaleString('es-PE')}</div>
                      </div>
                    ))
                  }
                </div>
